...
## v0.9 major restructuring + gameplay fixes pass
- **Fixed: Haggled Savings achievement was completely dead.** The tracking code had been accidentally wired into `accept_special_offer()` (the rare seller-event purchase flow) instead of the normal `buy_item()` function during an earlier pass — meaning it could never accumulate from ordinary haggling no matter how much you saved. Moved to the correct function.
- **Selling without checking Condition now carries real return risk** (+12% return chance) — representing the generic "not as described" risk of listing something you never actually inspected.
- **The stall-page "Rare-variant gamble" hint is now real**, not flavor text. It's locked in at the moment of Basic Research and directly sets your Deep Research odds — a bad-looking deal now genuinely has better rare-variant odds than a good-looking one, replacing the previous flat 20% baseline for researched items.
- **Test and Deep Research now show their odds directly on the button** (two-line button text), with results appearing as blue text in the same location as Researched Prices/Condition Checked — consistent styling for all research-type results. Also fixed `test_note` being referenced in the display code but never actually populated.
- **Renamed "Quick Look" to "Inspect"** everywhere (buttons, notes, shop track, tooltips).
- **Major layout restructuring**: removed the nested per-screen scrollboxes entirely (Stall, Stalls, Inventory, Shop, Achievements, Sold Today) in favour of one persistent page-level scroll. Cash/Carry/Storage/Reputation/nav now stay fixed at the top while everything else scrolls underneath as one continuous page, instead of a confined scrollbox-within-a-page. Fixed a scroll-reset bug this surfaced (nav clicks now properly snap back to the top again, since the scroll container is no longer recreated per screen).
- **Added a Patch Notes screen** (new nav button) — will be kept updated with what's changed each time changes ship.

## Runtime note
The project was statically checked and ZIP integrity was verified, but Godot itself is not installed in the build environment, so runtime behaviour has not been launch-tested here. This pass includes a genuinely major structural change (the scroll restructuring) — please test this specifically and carefully in the editor before deploying, since it touches how every screen renders. The mobile scale fix in particular is a best-effort based on live phone-test feedback in conversation, not something I could verify myself — please re-test after this update and report back if the sizing still needs tuning (the 700px threshold and 2.2x boost factor are reasonable starting guesses, not tuned values).
